...
contract Witlab is ERC721, ERC721URIStorage, Ownable, PriceConsumerV3 {
    ...
    mapping (address => uint) public uriTokenIDMap;
    ...
    function _safeMint() internal {
        ...
        uriTokenIDMap[to] = tokenId;
        
        hasMinted[to] = true;
    } 

    function mintUsingEth() public payable {
        require(msg.value == mintFeeInEth, "Minting fee is exactly 0.001 ether");
        require(hasMinted[msg.sender] == false, "Only one minting allowed per address");
        _safeMint();
    }
    ...
    function transferSBT(address _to, uint _tokenID) public {
    ...
    function getTokenPriceInETH(address _tokenAddress) public view returns(int){
        return getTokenPriceInUSD(_tokenAddress)/getTokenPriceInUSD(0xD4a33860578De61DBAbDc8BFdb98FD742fA7028e);
    }

    function getTokenPriceInUSD(address _tokenAddress) public view returns(int){
        return PriceConsumerV3.getLatestPrice(_tokenAddress) ;
    }   
